...
public class FriendsDao {
    ...
    public void addFriend(User requestingUser, User targetUser) {
        ...
        try {
            possibleFriendId = jdbcTemplate.queryForObject(sqlQuery, Integer.class, targetUserId, requestingUserId);
        } catch (RuntimeException e) {
            possibleFriendId = null;
        }

        if (Objects.equals(possibleFriendId, targetUserId)) {
            String sqlQueryUpdate = "update user_friends set confirmed = true where user_id = ? and friend_id = ?";
            String sqlQueryInsert = "insert into user_friends (user_id, friend_id, confirmed) values (?, ?, true)";
            ...
            String sqlQueryInsert = "insert into user_friends (user_id, friend_id, confirmed) values (?, ?, false)";
            jdbcTemplate.update(sqlQueryInsert,
                    requestingUserId,
                    targetUserId);
        }
    ...
    public List<User> getAllFriends(Integer userId) {
        String sqlQuery = "select * from users u join user_friends uf " +
                "on u.user_id = uf.user_id where uf.friend_id = ? and confirmed = true";
        List<User> friendsList = jdbcTemplate.query(sqlQuery, userRowMapper(), userId);
    ...
    public List<User> getCommonFriends(Integer id, Integer otherId) {
        String sqlQuery = "select u.user_id, u.name, u.login, u.email, u.birthday from users u " +
                ...
                "where User_id = ? and confirmed = true or user_id = ? and confirmed = true) " +
                "as uf group by uf.friend_id having count(*) > 1) as of on of.friend_id = u.user_id";
        List<User> commonFriends = jdbcTemplate.query(sqlQuery, userRowMapper(), id, otherId);
        if (commonFriends.isEmpty()) {
            throw new NullObjectException("У пользователей нет общих друзей!");
        }
